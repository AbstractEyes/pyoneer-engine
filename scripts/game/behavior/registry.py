...
On the tmx OBJECT, not on the class and not in a table:
    ...
    <property name="{behaviors}" value="player_input,topdown_move,animation_drive"/>
    <property name="{param}sprint_verb" value=""/>

That is a driven top-down body: `player_input` publishes the intent,
`topdown_move` spends it, `animation_drive` names the sequence. The second
line is a parameter override -- `player_input` declares `sprint_verb`, and an
empty value means this body cannot sprint. `topdown_move` itself takes no
parameters. A side-on body swaps `topdown_move` for `platformer_move`, whose
tuning (`gravity`, `jump_velocity`, ...) is read from an actors row named by
`{actor}`; every token's own parameter table is below.
...
The list is comma-separated and snake_case, and an unregistered token RAISES
at load. Run order comes from each behavior's declared `order`, not from the
list -- except between two behaviors at ONE order, which run in the order the
list names them (see "Run order" below). The map file is the whole truth: a
genre pack may supply a default list, but the editor materialises it into the
object when the object is added, so an object plays the same way whether or
not the editor has ever opened the map.
...
Most specific first, the same shape as `resolve_depth`:
    ...
    2. the `<key>` column of the actors row named by `{actor}` --
       consulted only for a parameter declared `source="actors"`
...
mask is per-CELL (a companion may declare finer sub-cells, but each is still
one mask) and tested at ONE anchor point rather than a box; blocking is
...
the map's collision once at bind and hands it to every entity it binds, by
either route. A map that declares neither a passability companion layer nor a
tileset's collision defaults bakes `None`, which means ungated -- a body on
such a map moves freely. The shipped `data/maps/starter.tmx` is NOT that map:
its `Floor` layer names `FloorCollision` as its passability companion, so the
demo's bodies are gated. The measured integration table below is the
authority on the wire; this paragraph is prose and can rot.

## Adding a behavior

1. Write the class in a module under `scripts/game/behavior/`, deriving
   `EntityBehavior`. Take the resolved parameters as constructor keywords
   named exactly for the keys the spec declares.
2. Declare its spec as a module constant BESIDE the class, so a parameter and
   the keyword it fills read in one screen. The shape, abridged from
   `scripts/game/behavior/movement.py`:

       PLATFORMER_MOVE = BehaviorSpec(
           name="platformer_move",  # plus the tag comment CLAUDE.md asks for
   ...
   A `source="actors"` parameter must be a column the pack named in `genres`
   declares -- `tools/check_behavior_docs.py` refuses it otherwise.
3. Register it: import the constant at the bottom of
   `scripts/game/behavior/registry.py` and add it to a `register_all(...)`
   call there. That file is the one place that says which token means which
   spec.
4. If it polls an input verb, bind the verb in `config/inputs.json` in the
   same change.
5. Add a check, and add it to `tools/check_all.py`'s roster.
6. `describe_all()` picks the new entry up with no further edit.

...
A thumb that fills its bar has zero travel, so dragging it divides by zero:

    ZeroDivisionError: float division by zero
    scroll.py: scroll_ratio = thumb_position / (bar.width - thumb.width)

`__scroll_thumb_bounds` guards the same quantity, so the drag path has to as
well. It is only reachable on a panel whose working area FITS -- a working
area larger than the panel on both axes hides it entirely, which is why the
fixtures below are sized exactly.
"""
...
# to fill. Shown unconditionally, it draws a black square in the bottom-right
# of any panel, scrollable or not.
...
# --------------------------------------------------------------------------
# Shrinking the window must not leave the scrollbars hanging off its edge or
# the backgrounds oversized. Three ways that happens, all asserted here:
#   1. EDGE-anchored scrollbars. A ScrollComponent is a full-panel overlay
#      that positions its parts at world_bounds.width - scroll_width, so
#      RIGHT-without-LEFT moves it (to x=-160) instead of resizing it.
#   2. arrows never repositioned on resize, left at x=362 in a 216-wide panel.
#   3. a background built from screen_area -- the virtual scroll extent, 420
#      tall -- rather than from the panel's own 160.

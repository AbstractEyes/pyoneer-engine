...
`docs/BEHAVIORS.md`'s hand-written preamble named a behavior token that did
not exist for months while matching its generator byte for byte.
...
# so it cannot get worse and cannot be forgotten. Each entry is
# (document, needle, why) and fails in BOTH directions: if the text disappears
# the pin is stale and must be deleted in the same change that fixed the
# defect; if new bad text appears it is not pinned and rule 6 catches it.
#
# EMPTY, and the mechanism stays. Its last entry was `tile_collision` in
# docs/BEHAVIORS.md's generated preamble, an unregistered token that made a
# map raise at load; the preamble now composes registered tokens and the pin
# was deleted in that same change. Rule 6 is what would catch a new one.
PINNED_STALE: list[tuple[str, str, str]] = []
...
`docs/map/` holds one tier-2 file per mapped module, every one emitted by
`tools/gen_map.py`; classifying them would demand a layer marker in each and
route each from CLAUDE.md, which is one navigation row per module for one
generated tree. Rule 10 checks that
...
def banner_for(tool: str) -> str:
    """The two lines every generated document opens with, naming `tool`.

    One shape for every generator -- this file's, `tools/gen_map.py`'s and the
    two foreign check generators' -- so a human opening ANY generated document
    sees the same first line and the regenerate command before any content.
    Rule 1 holds every L1 document to it.
    """
    return ("<!-- pyoneer-doc: L1 -->\n"
            f"<!-- GENERATED by {tool}. Do not edit. "
            f"Regenerate: .venv/Scripts/python.exe {tool} --write -->\n")


GEN_BANNER = banner_for("tools/check_docs.py")
...
# The FOREIGN half is the one with teeth: rule 7 already compares this file's
# own outputs byte for byte, but nothing else made BEHAVIORS.md and EVENTS.md
# open the way every other generated document does, and until 2026-09-16 they
# did not -- their regenerate command was buried mid-file.
_GENERATOR_OF = {rel: "tools/check_docs.py" for rel in GENERATED}
_GENERATOR_OF.update({rel: command.split()[0]
                      for rel, command in FOREIGN_GENERATED.items()})
expect_empty("every generated document opens with the one banner, naming its "
             "own generator",
             [f"{rel} does not open with {banner_for(tool)!r}"
              for rel, tool in sorted(_GENERATOR_OF.items())
              if not read(rel).startswith(banner_for(tool))])
...
# blind to values -- and it is the rule that caught `tile_collision`.

...
CHECKS = [
    ...
    ("nai", "the NovelAI sprite pipeline with every socket refused: one "
            ...
            "synthetic strips"),
    ("nai_ui", "the NovelAI composer window is only a composer: every "
               "button's argv parses under the LIVE cli parser, every "
               "Opus-tier number it shows is model's own, and its code "
               "never names the key, the probe flag, the transport or a "
               "process outside run_pane -- then the window itself is built "
               "off-screen and driven, proving both halves of each rule: a "
               "sending button starts nothing while a free one starts one "
               "thing, the arm needs a green dry run of its own request and "
               "drops on any edit, LOCK shuts the senders and only the "
               "senders, a refused character file shows the loader's own "
               "sentence and blocks every command naming it, and a decoy "
               "credential reaches no widget, argv or saved file -- and, "
               "through a fake child this check opens and closes by hand, "
               "that the clearance a send arms on is the argv the CHILD "
               "parsed and not the line the form has since recomposed, "
               "that a line which cannot run shuts its button and its menu "
               "item and says why instead of raising out of a Qt slot, that "
               "the line shown re-splits into the argv that runs under the "
               "platform's own quoting rule, that every key of "
               "request.FIXED_PARAMETERS is shown-and-locked or classified, "
               "and that nothing clips sideways at 1920, 1680, 1440 or "
               "1366 px"),

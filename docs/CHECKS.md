...
| `script_verbs` | the script.* verbs: exact inverses through nested elif arms, id-only addressing, every refusal the reader would make at load, and the seam -- the editor opens every script the engine reads, in THIS project, because a scene document declares the variables both sides use | Verify the `script.*` verbs: what they write, what they refuse, what undo |

...
# What to do next — one ranked list, as of `ce66ce5`

> **Filed under `docs/history/` on 2026-08-16 at `d8c303f`, and renamed for the
> commit it describes. Superseded by [`../NEXT.md`](../NEXT.md).**
>
> **ARCHIVE — do not navigate by this file.** This *was* `docs/NEXT.md`. It
> had already grown the STALE box below under its own byline, which is the
> moment a live document has become an archive and should be filed as one —
> a ranked list of work, most of it done, sitting at the address a reader is
> routed to for "what should I do next" is how a finished thing gets built
> twice. Route from [`../../CLAUDE.md`](../../CLAUDE.md).
...
> landed, and the roster has grown — [`CHECKS.md`](../CHECKS.md) is generated
> from it, so read the size there rather than from any sentence here. The
> ranked list below has not been re-measured against the tree since, so treat
> every entry in it as a claim to verify rather than a fact — which is the same

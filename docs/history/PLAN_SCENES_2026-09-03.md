...
> **Filed under `docs/history/` on 2026-09-16 at `99a1888`, frozen byte for byte from `docs/PLAN_SCENES.md` as that file stood, and named for the date its claims were measured. Superseded by [`../PLAN_SCENES.md`](../PLAN_SCENES.md), which now carries ONLY the unbuilt remainder. Kept because code cites its section numbers for the rationale behind what shipped -- every section number below is unchanged.**
>
> **What shipped, and where it diverged from this text** (re-measured 2026-09-16):
> - **Stage 1 shipped** -- `Session.ask` in `editor/core/session.py`, `describe_all(scopes=)` in `editor/core/commands.py`, `tools/check_relay.py`.
> - **Stage 4 shipped with TEN core ops, not eight** -- `play_sound` and `play_music` joined once `scripts/core/audio.py` existed; the registry is `scripts/game/flow/ops.py`. `demos/script.py` and `keeper_gate.json` were never built and are no longer needed: scripts run through `main.py`.
> - **Stage 5 shipped as `editor/ui/script_editor.py`** (`ScriptEditor` and `OpPicker` in one module), not `script_window.py` / `op_picker.py`, with the fourteen `script.*` verbs.
> - **Scene variables are read in `scripts/loaders/script_file.py`** (`SCENE_KEYS`, `load_vars`), not in a `scene_file.py`; the rest of Stage 6 is unbuilt.
> - **`EditorWindow.apply_response` was NOT collapsed onto `Session.apply_response`** -- it is a deliberate second door, and the scope gate lives in `read_response` so both doors share it. Section 5a.6 and Stage 8 prescribe the collapse; do not act on that.
>
> **ARCHIVE — do not navigate by this file.** Its counts, verb totals,
> `map:test` scopes and status claims describe the tree it was written
> against and are not re-measured. The live unbuilt remainder is
> [`../PLAN_SCENES.md`](../PLAN_SCENES.md); route from
> [`../../CLAUDE.md`](../../CLAUDE.md), and come here only for "why was it
> built this way".

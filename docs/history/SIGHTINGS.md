...
| 20 | 2026-09-19 | — | The rating-tag rule. The recipe and character routes searched the LOWER-CASED text with `RATING_RX`, and the new request-file route (`tools/nai/spec.py`) searched the raw text, so `Rating:General` in a request file's prompt or character caption passed `load_request`. Guard condition 7, the one gate every route passes, was a third spelling: the literal `"rating:" in text` on a character caption (`rating :explicit` passed too), and no look at the base caption before its closing tail at all. Found when the pixel editor's copy of the rule was compared against all three. Closed with ONE function, `model.rating_tag_in`, which recipes, characters, spec and the guard all call. `tools/check_nai.py` asserts by AST that no other module names `RATING_RX`, and mutating the lower-casing out of `rating_tag_in` turns it red. |
